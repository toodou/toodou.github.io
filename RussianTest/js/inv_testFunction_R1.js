...
//選擇題庫
function partSelect() {
    var part = document.getElementById("vv").value;
    part.checked = true;
}
//讀寫整個資料庫
...
function printQuestion(numQ) {
    ...
    for (var i = 0; i < numQ; i++) {
        ...
        orderNo = i;
        outQ += '<tr><td>' + orderNo + '</td><td>' + DATA[indexQ][2] + '</td><td><input type="text" id ="q' + i + '"></td></tr>';
        showQuestion.innerHTML = outQ + '</table>';
        //record the correct answer into array
        indQue.push(indexQ);
        quesArr.push(DATA[indexQ][2]);
        ansArr.push(DATA[indexQ][1]);
        ansArrR.push(DATA[indexQ][1]);
    }
}
//宣告單元
...
function checkAns(numQ, indQue, ansArr) {
    ...
    for (var i = 0; i < numQ; i++) {
        ...
        if (yourAns[i] != ansArrR[i]) {
            searchLink = '"https://translate.google.com.tw/?hl=zh-TW#ru/en/' + quesArr[i];

            outCA += 'You answer is ' + yourAns[i] + ' ; Correct answer is ' + ansArrR[i] + ';<br/>'; //+ '(' + ansArr[i] + ')' + ';<br/>';
            createCell(tbl.rows[i].insertCell(tbl.rows[i].cells.length), ansArrR[i], 'col');
            //createCellTW(tbl.rows[i].insertCell(tbl.rows[i].cells.length), ansArr[i], 'col');
        } else {
