...
function printQuestion(numQ) {
    ...
    for (var i = 0; i < numQ; i++) {
        ...
        orderNo = i;
        outQ += '<tr><td>' + orderNo + '</td><td>' + DATA[indexQ][3] + '</td><td><input type="text" id ="q' + i + '"></td></tr>';
        showQuestion.innerHTML = outQ + '</table>';
        //record the correct answer into array
        indQue.push(indexQ);
        quesArr.push(DATA[indexQ][3]);
...
function checkAns(numQ, indQue, ansArr) {
    ...
    for (var i = 0; i < numQ; i++) {
        ...
        if (yourAns[i] != ansArrR[i]) {
            ...
            outCA += 'You answer is ' + yourAns[i] + ' ; Correct answer is ' + ansArrR[i] + '(' + ansArr[i] + ')' + ';<br/>';
